...
class Ion_auth_model extends CI_Model
{
	...
	public function register_user($identity, $password, $email, $additional_data = array(), $group_id)
	{
		$this->trigger_events('pre_register');
	...
	**/
	...
	public function admin_notifikasi(){
        $this->db->select('a.*, b.title AS title, b.file as file, CONCAT(c.firstname," ",c.lastname) AS contributor');
        $this->db->from('dosen as a');
        $this->db->where('NIDN = 0');
        $this->db->join('kaprodi as b','b.id_kaprodi = a.id_kaprodi','left');
