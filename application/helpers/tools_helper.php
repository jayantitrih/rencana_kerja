<?php
	...
	function render_input_html($type='',$name='',$data='')
	{
		...
		if (is_array($type)) {
			...
			if (in_array($name, $auto_produk)) {
				$input_class = 'form-control auto-produk';
				$input 		.='<input type="hidden" name="id_produk" />';
			}elseif (in_array($name, $auto_users)) {
				$input_class = 'form-control auto-users';
				$input 		.='<input type="hidden" name="id_user" />';
			}else{
	...
	function set_date_interval($interval,$type='hari'){
		$satuan['hari']		='days';
		$satuan['bulan']	='month';
		$satuan['tahun']	='year';
		if (isset($satuan[$type])) {
			$type 			= $satuan[$type];
		}
    	$date = strtotime("+".$interval." ".$type, time());
